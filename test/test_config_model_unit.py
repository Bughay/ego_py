"""Unit tests for egoai.llm.config — the ConfigModel config schema.
...
class TestValidation(unittest.TestCase):
    ...
    def test_print_output_must_be_bool(self):
        for bad in (1, 0, "true", None, [], {}):
            with self.subTest(value=bad):
                with self.assertRaisesRegex(TypeError, "print_output.*bool"):
                    ConfigModel.from_dict({"print_output": bad})

    def test_print_output_bool_round_trip(self):
        for good in (True, False):
            with self.subTest(value=good):
                self.assertEqual(
                    ConfigModel.from_dict({"print_output": good}).to_dict(),
                    {"print_output": good},
                )
