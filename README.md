...
Most LLM libraries make you think about providers, payloads, and message formats. This one doesn't. Everything — a raw model call, a tool-using agent, or a whole multi-agent pipeline — is just an object you talk to:
...
You describe the problem; the object turns it into a solution. `EgoAgent` picks a strategy (`react`, `plan-execute`, `plan-react`, `agent-swarm`) and a provider (`deepseek*`, `grok*`), runs the loop — LLM calls, tool executions, memory — and hands back the answer. A raw `LLM` does the same for single calls. The problem goes in, the solution comes out — that's the whole idea.
...
| `context_manager` | `dict` or `None` | Automatic context management with two independent switches, each `None` by default (= disabled): `summarize` — when cumulative token usage reaches this value, the conversation is compressed into a single summary message (2000-token budget) and the counter resets. `max_iteration` — after this many `one_shot` turns, old non-system messages are trimmed, keeping the system prompt(s) and the 4 most recent messages. |
...
Every object exposes exactly five verbs: `one_shot()`, `extract()`, `classify()`, `summarize()` on every LLM, and `run()` on every agent. That's the whole API. It is minimal on purpose: one verb to talk, three verbs to structure output, one verb to solve a task. Every bigger workflow — research pipelines, triage systems, multi-stage agents — is just these verbs composed in plain Python (see "How to use the library" above). Fewer verbs means less to learn, less to debug, and nothing to fight when you chain stages.
...
Because memory is a plain list of message dicts, moving a conversation between objects is a slice assignment — no serialization, no special transfer API:
...
- **`summarize`** — when cumulative token usage reaches this number, the whole conversation so far is compressed into a single summary message (2000-token budget) and the counter resets. Thousands of tokens of back-and-forth become one short paragraph.
- **`max_iteration`** — after this many `one_shot()` turns, `trim_memory()` drops the old non-system messages, keeping the system prompt(s) and the 4 most recent messages, and the counter resets. The conversation keeps going with fresh room.
