...
class GrokLLM(BaseLLM):
    """
    Grok (xAI) API wrapper. Inherits memory management, tool handling,
    and chat loop from BaseLLM. Adds Grok-specific:
    - reasoning_effort (thinking mode, including xAI's "xhigh")
    - temperature fixed at 1.0 (Grok requirement; omitted from payload)
    - API key required: XAI_API_KEY env var (raises if missing)
...
#-------------------------- properties -----------------------------------------
    ...
    def model(self, value: str):
        ...
        if value not in allowed_models:
            raise ValueError(f"Available Grok models: {allowed_models}")
        self._model = value

    # ---------- Grok‑specific property ----------
    @property
...
#-------------------------- private helpers -------------------------------------
    ...
    def _get_api_key(self) -> str:
        ...
        #     key = input("Enter your xAI API key: ").strip()
        #     if not key:
...
#-------------------------- public methods --------------------------------------
    ...
    def one_shot(self) -> Dict[str, Any]:
        ...
        # -------- Build assistant message and append to memory --------
        ...
        # -------- Parse tool calls for the return dict --------
        tool_calls = []
        if message.tool_calls:
            for tc in message.tool_calls:
                try:
                    ...
                    args = {}  # fallback if JSON is malformed
                tool_calls.append({
                    "id": tc.id,
                    "name": tc.function.name,
                    "arguments": args,  # parsed dict
                })

        # Grok returns reasoning content in a separate attribute
